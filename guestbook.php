...
$query="insert into  guest_booking(roomno,seater_type,feespd,foodstatus,stayfrom,duration,regno,Name,gender,contactno,emailid,egycontactno,guardianName,guardianRelation,guardianContactno,corresAddress,corresCIty,corresState,corresPincode,pmntAddress,pmntCity,pmnatetState,pmntPincode) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
$stmt = $mysqli->prepare($query);
$rc=$stmt->bind_param('iiiisiissssisissssssiss',$roomno,$seater_type,$feespd,$foodstatus,$stayfrom,$duration,$regno,$name,$gender,$contactno,$emailid,$emcntno,$gurname,$gurrelation,$gurcntno,$caddress,$ccity,$cstate,$cpincode,$paddress,$pcity,$pstate,$ppincode);
$stmt->execute();
...
<head>
	...
	<title>guest Hostel Registration</title>
				...
				<div class="row">
					<div class="col-md-12">
						...
						<div class="row">
							...
							<?php
$uid=$_SESSION['login'];
							 $stmt=$mysqli->prepare("SELECT emailid FROM guest_booking WHERE emailid=? ");
...
<div class="col-sm-8">
<input type="text" name="seater_type" id="seater_type"  class="form-control"  >
</div>
</div>
...
<div class="col-sm-8">
<input type="text" name="fpd" id="fpd"  class="form-control" >
</div>
</div>
...
<input type="radio" value="1" name="foodstatus"> With Food(Rs 500.00 Per day Extra)
</div>
</div>	
...
<?php	
$aid=$_SESSION['id'];
	$ret="select * from guest where id=?";
